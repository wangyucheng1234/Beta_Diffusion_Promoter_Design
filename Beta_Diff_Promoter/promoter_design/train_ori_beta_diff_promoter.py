    ...
    for t in test_datasets:
        ...
        for i in range(5):
            ...
            samples.append(bd_sampler(net = score_model, latents = latents.cuda(), class_labels = t[:, :, 4:5].cuda(), randn_like=torch.randn_like,
                    num_steps=100, alpha_min=None, eta=args.eta, sigmoid_start=args.sigmoid_start, sigmoid_end=args.sigmoid_end, sigmoid_power=args.sigmoid_power, start_step=None,Scale=0.5,Shift=0.4)[0].detach().cpu().numpy())
